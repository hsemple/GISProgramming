...
ArcPy provides access to all the tools in ArcToolbox.  Tools are accessed as functions using the following syntax :  
...
Calculate Aspect and Hillshade
...
try:
 ...
 Learn more about Cursor (Links to an external site.)

    


b.  Extract DEMs for each of Michigan's 83 Counties from a Single Michigan DEM
...
c.  Calculate Slope for all Counties in Michigan 
...
d. Watershed Delineation  (Download Data)
...
References
*Terrain Analysis with ArcPyPreview the document
*Terrain Analysis and Geoprocessing Rasters with GDALPreview the document
* ArcPy's Class Diagrams (Links to an external site.)Links to an external site.(pdf)
